...
if not os.getenv("OPENAI_API_KEY"):
    pytest.skip("OpenAI API 키가 없어 Task 6.2 워크플로우 테스트를 건너뜁니다.", allow_module_level=True)

sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))


def test_retrieve_yes():
    """검색 필요 O: 구체적인 대사증후군 관리 질문"""
    ...
    try:
        ...
        print(
            f"retrieve_decision: {final_state.get('metadata', {}).get('retrieve_decision')}"
        )
        print(
            f"retrieve_reason: {final_state.get('metadata', {}).get('retrieve_reason')}"
        )
        ...
        # 검증
        assert (
            final_state.get("should_retrieve") == True
        ), f"검색 필요성 판단 실패. decision={final_state.get('metadata', {}).get('retrieve_decision')}, reason={final_state.get('metadata', {}).get('retrieve_reason')}"
        assert len(final_state.get("internal_docs", [])) > 0, "내부 검색 수행되지 않음"
        assert len(final_state.get("answer", "")) > 0, "답변 생성 실패"

        print(f"\n✅ should_retrieve: {final_state['should_retrieve']}")
...
def test_retrieve_no():
    ...
    try:
        ...
        # 검증
        assert (
            final_state.get("should_retrieve") == False
        ), f"검색 스킵 실패. decision={final_state.get('metadata', {}).get('retrieve_decision')}"
        assert (
